...
When **reviewing recordings** in gcrdings, we recommend using **computer speakers** or **wired headphones** rather than Bluetooth headphones for accurate playback.
...
1. **gcrdings records at 48kHz** (professional audio standard)
...
- ❌ **Reviewing gcrdings recordings** - Use wired/speakers instead
...
### This is Not a gcrdings Bug

The issue is in **macOS's Bluetooth audio stack**, not in gcrdings's recording engine.
...
**A**: This is a macOS/Bluetooth limitation, not a gcrdings bug. We've correctly recorded at 48kHz.
...
**A**: Yes! gcrdings now shows a warning when Bluetooth headphones are active during playback.
...
**Applies To**: gcrdings v0.0.5+ on macOS
