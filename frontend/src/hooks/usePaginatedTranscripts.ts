...
export function mergePaginatedTranscripts(
    existing: Transcript[],
    incoming: Transcript[],
): Transcript[] {
    const existingIds = new Set(existing.map(({ id }) => id));
    return [...existing, ...incoming.filter(({ id }) => !existingIds.has(id))].sort(
...
export function usePaginatedTranscripts({
    ...
    const loadTranscriptsAtOffset = useCallback(async (
        ...
        try {
            ...
            if (append) {
                setTranscripts(prev => mergePaginatedTranscripts(prev, newTranscripts));
            } else {
