  ...
  useEffect(() => {
    ...
    const setupRecordingStoppedListener = async () => {
      try {
        ...
        unlistenWarning = await listen<{ message: string }>(
    ...
    try {
      ...
      if (isCallApi && transcriptionComplete == true) {
        ...
        try {
          ...
          setStatus(RecordingStatus.PROCESSING_TRANSCRIPTS, 'Transcribing saved recording...');
          const transcriptConfig = await configService.getTranscriptConfig();
          let resolveTranscription!: () => void;
          let rejectTranscription!: (error: Error) => void;
          const transcriptionFinished = new Promise<void>((resolve, reject) => {
            resolveTranscription = resolve;
            rejectTranscription = reject;
          });
          const unlistenRetranscriptionComplete = await listen<{
            meeting_id: string;
            diarization_completed: boolean;
            failed_origins: string[];
          }>(
            'retranscription-complete',
            (event) => {
              if (event.payload.meeting_id === meetingId) {
                if (event.payload.failed_origins.length > 0) {
                  toast.warning('Transcript ready with missing audio', {
                    description: `Unavailable source: ${event.payload.failed_origins.join(', ')}. Use Enhance to retry.`,
                  });
                } else if (!event.payload.diarization_completed) {
                  toast.warning('Transcript ready without speaker labels', {
                    description: 'Use Enhance to retry speaker identification.',
                  });
                }
          ...
          const unlistenRetranscriptionError = await listen<{ meeting_id: string; error: string }>(
            'retranscription-error',
            (event) => {
              if (event.payload.meeting_id === meetingId) {
                rejectTranscription(new Error(event.payload.error));
              }
            }
          );
          ...
          try {
            await invoke('start_retranscription_command', {
              ...
              provider: transcriptConfig.provider,
            });
            await transcriptionFinished;
          } finally {
            unlistenRetranscriptionComplete();
            unlistenRetranscriptionError();
          }
          ...
          toast.success('Recording saved successfully!', {
            ...
            action: {
              label: 'View Meeting',
              onClick: () => {
                if (navigate(`/meeting-details?id=${meetingId}`)) {
          ...
          setTimeout(() => {
            if (navigate(`/meeting-details?id=${meetingId}&source=recording`)) {
              clearTranscripts();
