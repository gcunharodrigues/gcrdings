...
export function useCopyOperations({
  ...
  const fetchAllTranscripts = useCallback(async (meetingId: string): Promise<Transcript[]> => {
    try {
      log.debug('📊 Fetching all transcripts for copying:', meetingId);
      ...
      log.debug(`✅ Fetched ${allData.transcripts.length} transcripts from database for copying`);
  ...
  const handleCopyTranscript = useCallback(async () => {
    ...
    log.debug('📊 Fetching all transcripts for copying...');
    ...
    log.debug(`✅ Copying ${allTranscripts.length} transcripts to clipboard`);
  ...
  const handleCopySummary = useCallback(async () => {
    try {
      ...
      if (!summaryMarkdown && aiSummary) {
        ...
        log.debug('📝 Converted legacy format, length:', summaryMarkdown.length);
      }
