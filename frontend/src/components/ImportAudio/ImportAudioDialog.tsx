...
export function ImportAudioDialog({
  ...
  const { refetchMeetings, navigate } = useSidebar();
  ...
  const handleImportComplete = useCallback((result: ImportResult) => {
    toast.success('Media imported. Audio is ready for processing.');

    // Refresh meetings list then navigate to the imported meeting
    refetchMeetings();
    onComplete?.();
    onOpenChange(false);
    navigate(`/meeting-details?id=${result.meeting_id}`);
  }, [navigate, refetchMeetings, onComplete, onOpenChange]);
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {isProcessing
              ...
              : 'Preserve an audio or video file and prepare its audio for a new Session'}
          </DialogDescription>
        </DialogHeader>
            ...
            <>
              {fileInfo ? (
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                  ...
                  <p className="text-sm text-gray-500 mt-2">MP4, MOV, M4V, M4A, WAV, MP3, FLAC, OGG, AAC</p>
                  <p className="text-xs text-gray-500 mt-1">
                    Importing the same file again creates a separate Session.
                  </p>
                </div>
              )}
            </>
