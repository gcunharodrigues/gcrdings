  ...
  return (
    <section aria-labelledby="findings-heading" className="flex min-h-0 flex-1 flex-col overflow-y-auto p-4">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3">
        <div>
          <h1 id="findings-heading" className="text-lg font-semibold text-foreground">Grounded findings</h1>
          <p className="text-xs text-muted-foreground">Every accepted finding resolves to a playable principal-transcript passage.</p>
        </div>
        {processing ? (
      ...
      <p aria-live="polite" className="mt-3 text-sm text-muted-foreground">{loadError ?? (record ? generationMessage(record.generation_status, record.error_code ?? undefined) : "Loading local findings…")}</p>

      {record?.generated && record.generation_status === "completed" && (
        typeMatches ? (
          <div className="mt-5">
            {mode.shape !== "prose" && <p className="mb-4 leading-7 text-foreground">{record.generated.summary}</p>}
            <ShapeView record={record} onSeek={onSeek} />
          </div>
        ) : (
          <p className="mt-5 rounded border border-dashed p-4 text-sm text-foreground/90">
      ...
      {record?.generated && record.generation_status === "completed" && typeMatches && (
        <p className="mt-4 text-xs text-muted-foreground/70">Showing the {RECORD_SHAPES[mode.shape].label.toLowerCase()} layout.</p>
      )}
    </section>
  );
}
