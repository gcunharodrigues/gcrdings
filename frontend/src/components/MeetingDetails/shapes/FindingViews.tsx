      ...
      {findingGroups(record.generated!).map((group) => (
        group.findings.length > 0 && (
          <section key={group.label}>
            <h2 className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{group.label}</h2>
...
export function ProseView({ record, onSeek }: ShapeProps) {
  return (
    <div className="space-y-5">
      <p className="leading-7 text-foreground">{record.generated!.summary}</p>
      {findingGroups(record.generated!).map((group) => (
        group.findings.length > 0 && (
          <section key={group.label}>
            <h2 className="mb-2 font-semibold text-foreground">{group.label}</h2>
            {group.findings.map((finding, index) => (
              <p key={`${finding.label}-${index}`} className="mb-3 leading-7 text-foreground">
      ...
      <table className="w-full min-w-[34rem] border-collapse text-sm">
        ...
        <tbody>
          {rows.map(({ group, finding }, index) => (
            <tr key={`${finding.label}-${index}`} className="border-b border-border align-top">
              <td className="py-2.5 pr-3 text-xs text-muted-foreground">{group}</td>
...
export function TimelineView({ record, onSeek }: ShapeProps) {
  ...
  if (ordered.length === 0) {
    return <p className="rounded border border-dashed p-4 text-sm text-muted-foreground">No finding carries a timestamp yet.</p>;
  }
      ...
      {ordered.map(({ group, finding }, index) => {
        ...
        return (
          <li key={`${finding.label}-${index}`} className="relative">
            <span className="absolute -left-[1.4rem] top-1.5 size-2 rounded-full bg-blue-600 ring-2 ring-card" aria-hidden="true" />
            <button
              type="button"
              onClick={() => onSeek(timestampMs)}
            ...
            <p className="text-sm leading-6 text-foreground/90">{finding.detail}</p>
          </li>
        );
      })}
    </ol>
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border border-border bg-muted/40 px-3 py-2">
        ...
        <p className="text-xs text-muted-foreground">{generatedFindings(record.generated!).length} findings</p>
      </div>
              ...
              <ul className="mt-1.5 space-y-1.5 border-l border-border pl-4">
                {group.findings.map((finding, index) => (
                  <li key={`${finding.label}-${index}`}>
                    <p className="text-sm text-foreground">{finding.label}</p>
                    {finding.evidence[0] && (
                      <button
                        type="button"
                        onClick={() => onSeek(finding.evidence[0].timestamp_ms)}
