...
export function RecordingSettings({ onSave }: RecordingSettingsProps) {
  ...
  const handleSpeakerPreferenceChange = async (
    changes: Pick<RecordingPreferences, 'local_speaker_name'> | Pick<RecordingPreferences, 'microphone_is_shared'>
  ) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4 border-t pt-6">
        <h4 className="text-base font-medium text-gray-900">Microphone Participants</h4>
        <div className="flex items-center justify-between p-4 border rounded-lg">
          <div className="flex-1">
            <div className="font-medium">Shared microphone</div>
            <div className="text-sm text-gray-600">
              Enable when more than one person speaks into this microphone.
            </div>
          </div>
          <Switch
            checked={preferences.microphone_is_shared}
            onCheckedChange={(enabled) => handleSpeakerPreferenceChange({ microphone_is_shared: enabled })}
            disabled={saving}
          />
        </div>
        {!preferences.microphone_is_shared && (
          <div className="space-y-2">
            <label htmlFor="local-speaker-name" className="text-sm font-medium text-gray-700">
              Your transcript name
            </label>
            <Input
              id="local-speaker-name"
              value={preferences.local_speaker_name}
              onChange={(event) => setPreferences(prev => ({ ...prev, local_speaker_name: event.target.value }))}
              onBlur={() => handleSpeakerPreferenceChange({ local_speaker_name: preferences.local_speaker_name })}
              disabled={saving}
            />
          </div>
        )}
