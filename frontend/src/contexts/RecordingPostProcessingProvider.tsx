...
export function RecordingPostProcessingProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      try {
        ...
        unlistenFn = await listen<boolean>('recording-stop-complete', (event) => {
          log.debug('[RecordingPostProcessing] Received recording-stop-complete event:', event.payload);
        ...
        log.debug('[RecordingPostProcessing] Event listener set up successfully');
      } catch (error) {
    ...
    return () => {
      if (unlistenFn) {
        log.debug('[RecordingPostProcessing] Cleaning up event listener');
        unlistenFn();
      }
    };
  }, [handleRecordingStop]);
