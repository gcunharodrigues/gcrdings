...
use crate::{
    database::{
        ...
        repositories::{
            ...
            principal_record::{
                ParticipantChange, PrincipalRecordError, PrincipalRecordReceipt,
                PrincipalRecordRepository, SavePrincipalRecord, TranscriptChange,
            },
...
enum SessionAudioError {
    #[error("session audio is missing")]
    Missing,
    #[error("session audio is empty")]
    Empty,
    #[error("session audio target is unsafe")]
    Unsafe,
}

fn resolve_session_audio(
    recording_folder: &std::path::Path,
) -> Result<std::path::PathBuf, SessionAudioError> {
    ...
    let canonical_folder = recording_folder
        .canonicalize()
        .map_err(|_| SessionAudioError::Missing)?;
    if !canonical_folder.is_dir() {
        return Err(SessionAudioError::Unsafe);
    }

    for name in CANDIDATES {
        let candidate = recording_folder.join(name);
        if !candidate.exists() {
            continue;
        }
        let canonical_candidate = candidate
            .canonicalize()
            .map_err(|_| SessionAudioError::Unsafe)?;
        if !canonical_candidate.starts_with(&canonical_folder) || !canonical_candidate.is_file() {
            return Err(SessionAudioError::Unsafe);
        }
        if canonical_candidate
            ...
            == 0
        {
            return Err(SessionAudioError::Empty);
        }
        return Ok(canonical_candidate);
    }

    Err(SessionAudioError::Missing)
}
...
pub struct MeetingTranscript {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub alignment_decision: Option<String>,
...
pub struct SessionRecord {
    ...
    pub participants: Vec<crate::database::models::Participant>,
    pub transcripts: Vec<MeetingTranscript>,
}
...
mod session_audio_tests {
    use super::{resolve_session_audio, SessionAudioError};
    use std::fs;

    #[test]
    fn resolves_the_registered_mixed_track() {
        let root = tempfile::tempdir().unwrap();
        let audio = root.path().join("audio.mp4");
        fs::write(&audio, b"mixed track").unwrap();

        assert_eq!(
            resolve_session_audio(root.path()).unwrap(),
            audio.canonicalize().unwrap()
        );
    }

    #[test]
    fn rejects_missing_empty_and_non_file_targets() {
        let missing = tempfile::tempdir().unwrap();
        assert_eq!(
            resolve_session_audio(missing.path()),
            Err(SessionAudioError::Missing)
        );

        let empty = tempfile::tempdir().unwrap();
        fs::write(empty.path().join("audio.mp4"), []).unwrap();
        assert_eq!(
            resolve_session_audio(empty.path()),
            Err(SessionAudioError::Empty)
        );

        let directory = tempfile::tempdir().unwrap();
        fs::create_dir(directory.path().join("audio.mp4")).unwrap();
        assert_eq!(
            resolve_session_audio(directory.path()),
            Err(SessionAudioError::Unsafe)
        );
    }

    #[cfg(unix)]
    #[test]
    fn rejects_a_symlink_that_escapes_the_recording_folder() {
        ...
        assert_eq!(
            resolve_session_audio(recording.path()),
            Err(SessionAudioError::Unsafe)
        );
    }
}
...
#[tauri::command]
// Tauri IPC fields intentionally mirror the stable model-settings contract.
...
pub async fn api_save_model_config<R: Runtime>(
    ...
    if let Some(key) = api_key {
        if !key.is_empty() && provider != "custom-openai" {
            use crate::providers::{summary_credential_account, CredentialStore};
            crate::providers::keychain::MacKeychain::default()
                .save(&summary_credential_account(&provider), &key)
...
pub async fn api_save_transcript_config<R: Runtime>(
    ...
    if let Some(key) = api_key {
        if !key.is_empty() {
            ...
            let account = format!("transcript:{}", provider.to_ascii_lowercase());
            crate::providers::keychain::MacKeychain::default()
                .save(&account, &key)
                .map_err(|error| error.to_string())?;
        }
...
pub async fn api_delete_api_key<R: Runtime>(
    ...
    use crate::providers::CredentialStore;
    crate::providers::keychain::MacKeychain::default()
        .remove(&crate::providers::summary_credential_account(&provider))
        .map_err(|error| error.to_string())?;
        ...
        Ok((transcripts, total_count)) => {
            ...
            let meeting_transcripts = transcripts
                ...
                .map(|t| MeetingTranscript {
                    ...
                    ambiguity_group_id: t.ambiguity_group_id,
                    alignment_decision: t.alignment_decision,
...
#[tauri::command]
pub async fn api_get_session_record<R: Runtime>(
    _app: AppHandle<R>,
    meeting_id: String,
    state: tauri::State<'_, AppState>,
) -> Result<SessionRecord, PrincipalRecordError> {
    let record = PrincipalRecordRepository::get(state.db_manager.pool(), &meeting_id).await?;
    Ok(SessionRecord {
        ...
        transcripts: record
            ...
            .map(|transcript| MeetingTranscript {
                ...
                source_origin: transcript.source_origin,
                speaker_cluster_id: transcript.speaker_cluster_id,
                ambiguity_group_id: transcript.ambiguity_group_id,
                alignment_decision: transcript.alignment_decision,
...
#[tauri::command]
pub async fn api_read_session_audio<R: Runtime>(
    _app: AppHandle<R>,
    meeting_id: String,
    state: tauri::State<'_, AppState>,
    ...
    let folder_path: Option<String> =
        sqlx::query_scalar("SELECT folder_path FROM meetings WHERE id = ?")
            ...
            .await
            .map_err(|_| "Failed to resolve Session audio".to_string())?
            .flatten();
    let folder_path = folder_path.ok_or_else(|| "Session audio is unavailable".to_string())?;
    let audio_path = resolve_session_audio(std::path::Path::new(&folder_path))
        .map_err(|error| error.to_string())?;
    std::fs::read(audio_path).map_err(|_| "Failed to read Session audio".to_string())
}

#[tauri::command]
pub async fn api_save_session_record<R: Runtime>(
    _app: AppHandle<R>,
    meeting_id: String,
...
pub async fn api_save_transcript<R: Runtime>(
    ...
    log_info!(
        "api_save_transcript called for meeting: {}, transcripts: {}, has_folder_path: {}, auth_token: {}",
        meeting_title,
        transcripts.len(),
        folder_path.is_some(),
        auth_token.is_some()
    ...
    match meeting {
        Some(m) => {
            if let Some(folder_path) = m.folder_path {
                ...
                if !path.exists() {
                    log_warn!("Recording folder does not exist");
                ...
                log_info!("Successfully opened meeting folder");
                Ok(())
            } else {
...
#[tauri::command]
// Tauri IPC fields intentionally mirror the stable custom-provider contract.
#[allow(clippy::too_many_arguments)]
pub async fn api_save_custom_openai_config<R: Runtime>(
    ...
    if let Some(key) = api_key.as_deref().filter(|key| !key.trim().is_empty()) {
        use crate::providers::{summary_credential_account, CredentialStore};
        crate::providers::keychain::MacKeychain::default()
            .save(&summary_credential_account("custom-openai"), key)
            .map_err(|error| error.to_string())?;
    }
