...
pub(crate) fn whisper_model_source(model_name: &str) -> Option<(&'static str, &'static str)> {
    match model_name {
        "tiny" => Some(("https://huggingface.co/ggerganov/whisper.cpp/resolve/5359861c739e955e79d9a303bcbc70fb988958b1/ggml-tiny.bin", "be07e048e1e599ad46341c8d2a135645097a538221678b7acdd1b1919c6e1b21")),
        "base" => Some(("https://huggingface.co/ggerganov/whisper.cpp/resolve/5359861c739e955e79d9a303bcbc70fb988958b1/ggml-base.bin", "60ed5bc3dd14eea856493d334349b405782ddcaf0028d4b5df4088345fba2efe")),
        "small" => Some(("https://huggingface.co/ggerganov/whisper.cpp/resolve/5359861c739e955e79d9a303bcbc70fb988958b1/ggml-small.bin", "1be3a9b2063867b937e64e2ec7483364a79917e157fa98c5d94b5c1fffea987b")),
        "medium" => Some(("https://huggingface.co/ggerganov/whisper.cpp/resolve/5359861c739e955e79d9a303bcbc70fb988958b1/ggml-medium.bin", "6c14d5adee5f86394037b4e4e8b59f1673b6cee10e3cf0b11bbdbee79c156208")),
        "large-v3-turbo" => Some(("https://huggingface.co/ggerganov/whisper.cpp/resolve/5359861c739e955e79d9a303bcbc70fb988958b1/ggml-large-v3-turbo.bin", "1fc70f774d38eb169993ac391eea357ef47c88757ef72ee5943879b7e8e2bc69")),
        "large-v3" => Some(("https://huggingface.co/ggerganov/whisper.cpp/resolve/5359861c739e955e79d9a303bcbc70fb988958b1/ggml-large-v3.bin", "64d182b440b98d5203c4f9bd541544d84c605196c4f7b845dfa11fb23594d1e2")),
        "tiny-q5_1" => Some(("https://huggingface.co/ggerganov/whisper.cpp/resolve/5359861c739e955e79d9a303bcbc70fb988958b1/ggml-tiny-q5_1.bin", "818710568da3ca15689e31a743197b520007872ff9576237bda97bd1b469c3d7")),
        "base-q5_1" => Some(("https://huggingface.co/ggerganov/whisper.cpp/resolve/5359861c739e955e79d9a303bcbc70fb988958b1/ggml-base-q5_1.bin", "422f1ae452ade6f30a004d7e5c6a43195e4433bc370bf23fac9cc591f01a8898")),
        "small-q5_1" => Some(("https://huggingface.co/ggerganov/whisper.cpp/resolve/5359861c739e955e79d9a303bcbc70fb988958b1/ggml-small-q5_1.bin", "ae85e4a935d7a567bd102fe55afc16bb595bdb618e11b2fc7591bc08120411bb")),
        "medium-q5_0" => Some(("https://huggingface.co/ggerganov/whisper.cpp/resolve/5359861c739e955e79d9a303bcbc70fb988958b1/ggml-medium-q5_0.bin", "19fea4b380c3a618ec4723c3eef2eb785ffba0d0538cf43f8f235e7b3b34220f")),
        "large-v3-turbo-q5_0" => Some(("https://huggingface.co/ggerganov/whisper.cpp/resolve/5359861c739e955e79d9a303bcbc70fb988958b1/ggml-large-v3-turbo-q5_0.bin", "394221709cd5ad1f40c46e6031ca61bce88931e6e088c188294c6d5a55ffa7e2")),
        "large-v3-q5_0" => Some(("https://huggingface.co/ggerganov/whisper.cpp/resolve/5359861c739e955e79d9a303bcbc70fb988958b1/ggml-large-v3-q5_0.bin", "d75795ecff3f83b5faa89d1900604ad8c780abd5739fae406de19f23ecd98ad1")),
        _ => None,
    }
}

pub(crate) async fn validate_whisper_model_file(
    ...
    if !header.starts_with(b"ggml")
        && !header.starts_with(b"GGUF")
        && !header.starts_with(b"ggmf")
        && !header.starts_with(b"lmgg")
        && !header.starts_with(b"FUGU")
        && !header.starts_with(b"fmgg")
    {
        return Err(anyhow!(
...
impl WhisperEngine {
                ...
                match std::fs::metadata(&model_path) {
                    Ok(metadata) => {
                        ...
                        if file_size_mb >= expected_min_size_mb && file_size_mb > 1 {
                            ...
                            let expected_sha256 = whisper_model_source(name)
                                .ok_or_else(|| anyhow!("Unsupported model: {}", name))?
                                .1;
                            match validate_whisper_model_file(&model_path, expected_sha256).await {
    ...
    pub async fn load_model(&self, model_name: &str) -> Result<()> {
        ...
        match model_info.status.clone() {
            ModelStatus::Available => {
                let expected_sha256 = whisper_model_source(model_name)
                    .ok_or_else(|| anyhow!("Unsupported model: {}", model_name))?
                    .1;
                if let Err(error) =
                    validate_whisper_model_file(&model_info.path, expected_sha256).await
                {
                    let file_size = fs::metadata(&model_info.path)
                        .await
                        .map(|m| m.len())
                        .unwrap_or(0);
                    if let Some(model) = self.available_models.write().await.get_mut(model_name) {
                        model.status = ModelStatus::Corrupted {
                            file_size,
                            expected_min_size: (model_info.size_mb as u64 * 1024 * 1024 * 9) / 10,
                        };
                    }
                    return Err(anyhow!(
    ...
    fn calculate_repetition_ratio(text: &str) -> f32 {
        ...
        let repeated_words: usize = word_counts
            .values()
            .map(|&count| count.saturating_sub(1usize))
            .sum();
    ...
    pub async fn download_model(
        ...
        let (model_url, expected_sha256) = whisper_model_source(model_name)
            .ok_or_else(|| anyhow!("Unsupported model: {}", model_name))?;
            ...
            let _ = fs::remove_file(&file_path).await;
            {
                ...
                if let Some(model_info) = models.get_mut(model_name) {
                    model_info.status = ModelStatus::Error(
                        "Downloaded model failed SHA-256 verification".to_string(),
                    );
                }
            }
...
mod integrity_tests {
    ...
    fn wrong_tiny_model(path: &std::path::Path) {
        let mut file = std::fs::File::create(path).unwrap();
        file.set_len(74 * 1024 * 1024).unwrap();
        file.seek(SeekFrom::Start(0)).unwrap();
        file.write_all(b"ggml0000").unwrap();
    }

    #[tokio::test]
    async fn discovery_rejects_valid_size_and_header_with_wrong_digest() {
        let dir = tempfile::tempdir().unwrap();
        wrong_tiny_model(&dir.path().join("ggml-tiny.bin"));
        ...
        let tiny = models.iter().find(|model| model.name == "tiny").unwrap();

        assert!(matches!(tiny.status, ModelStatus::Corrupted { .. }));
    }

    #[tokio::test]
    async fn load_rechecks_digest_and_marks_changed_file_corrupted() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("ggml-tiny.bin");
        std::fs::write(&path, b"ggml0000wrong digest").unwrap();
        let engine = WhisperEngine::new_with_models_dir(Some(dir.path().to_path_buf())).unwrap();
        ...
        let error = engine.load_model("tiny").await.unwrap_err();

        assert!(error.to_string().contains("failed integrity verification"));
        let models = engine.available_models.read().await;
        assert!(matches!(
            models["tiny"].status,
            ModelStatus::Corrupted { .. }
        ));
