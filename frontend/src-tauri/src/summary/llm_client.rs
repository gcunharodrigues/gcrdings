use crate::providers::{ProviderAdapter, ProviderConfiguration, ProviderError, SecretString};
...
const MAX_PROVIDER_RESPONSE_BYTES: usize = 512_000;

pub struct ProviderHttpAdapter {
    client: reqwest::Client,
}

impl Default for ProviderHttpAdapter {
    fn default() -> Self {
        Self {
...
#[async_trait]
impl ProviderAdapter for ProviderHttpAdapter {
    async fn test(
        &self,
    ...
    async fn send(
        ...
        let response = self
            ...
            .header("Idempotency-Key", transfer_id)
        ...
        while let Some(chunk) = stream.next().await {
            let chunk = chunk.map_err(|_| ProviderError::OutcomeUnknown)?;
            if body.len() + chunk.len() > MAX_PROVIDER_RESPONSE_BYTES {
                return Err(ProviderError::OutcomeUnknown);
            }
            body.extend_from_slice(&chunk);
        }
        serde_json::from_slice(&body).map_err(|_| ProviderError::OutcomeUnknown)
    }
}
...
// Provider request fields remain explicit at the single outbound-request boundary.
...
mod provider_http_tests {
    use super::*;
    use crate::providers::{ProviderAdapter, AGENT_HANDOFF_TASK};
    ...
    #[tokio::test]
    async fn provider_send_never_follows_redirects() {
        ...
        let server = tokio::spawn(async move {
            loop {
                ...
                let response = if request.starts_with("POST /source ") {
                    format!(
        ...
        let configuration = ProviderConfiguration {
            ...
            endpoint: format!("http://{address}/source"),
            model: "fixture".into(),
        ...
        assert_eq!(
            ProviderHttpAdapter::default()
                .send(
                    &configuration,
                    &SecretString::new("synthetic-secret"),
                    "transfer-id",
                    r#"{\"session\":\"private\"}"#,
                )
                .await,
    ...
    #[tokio::test]
    async fn accepted_post_with_dropped_response_is_outcome_unknown_and_has_idempotency_key() {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let address = listener.local_addr().unwrap();
        let server = tokio::spawn(async move {
            let (mut stream, _) = listener.accept().await.unwrap();
            ...
            let read = stream.read(&mut request).await.unwrap();
            let request = String::from_utf8_lossy(&request[..read]);
            assert!(request.contains("idempotency-key: durable-transfer-id"));
