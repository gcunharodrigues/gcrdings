...
use crate::providers::{summary_credential_account, CredentialStore, ProviderError, SecretString};
...
fn load_summary_credential(
    ...
    let provider = match provider {
        ...
        LLMProvider::Ollama => return Ok(None),
    };
    credentials.read(&summary_credential_account(provider))
}
...
impl SummaryService {
    ...
    // The background boundary mirrors the persisted summary job inputs.
    #[allow(clippy::too_many_arguments)]
    pub async fn process_transcript_background<R: tauri::Runtime>(
        ...
        // Parse provider
        let provider = match model_provider.parse::<LLMProvider>() {
            Ok(p) => p,
            Err(e) => {
        ...
        let summary_credential = match load_summary_credential(&MacKeychain::default(), &provider) {
            Ok(credential) if provider == LLMProvider::Ollama || credential.is_some() => credential,
            Ok(_) => {
        ...
        let final_api_key = summary_credential
            .as_ref()
            .map(|secret| std::str::from_utf8(secret.expose()))
            .transpose()
            .unwrap_or(None)
            .unwrap_or("");
        ...
        // Retained for the legacy provider call signature.
...
mod tests {
    ...
    use crate::providers::{
        summary_credential_account, CredentialStore, ProviderError, SecretString,
    };
    ...
    #[derive(Default)]
    struct MemorySummaryCredentials(Mutex<HashMap<String, Vec<u8>>>);

    impl CredentialStore for MemorySummaryCredentials {
    ...
    async fn summary_fixture() -> SqlitePool {
    ...
    #[tokio::test]
    async fn saved_standard_and_custom_credentials_reach_the_backend_summary_consumer_after_restart(
    ) {
        let pool = summary_fixture().await;
        let credentials = MemorySummaryCredentials::default();
        credentials
            .save(&summary_credential_account("openai"), "saved-standard")
            .unwrap();
        credentials
            .save(&summary_credential_account("custom-openai"), "saved-custom")
            .unwrap();

        let standard = load_summary_credential(&credentials, &LLMProvider::OpenAI)
            .unwrap()
            .expect("standard summary credential");
        let custom = load_summary_credential(&credentials, &LLMProvider::CustomOpenAI)
            .unwrap()
            .expect("custom summary credential");
        assert_eq!(standard.expose(), b"saved-standard");
        assert_eq!(custom.expose(), b"saved-custom");
        assert!(SettingsRepository::get_api_key(&pool, "openai")
            .await
            .unwrap()
            .is_none());
    }
    ...
    #[tokio::test]
    async fn migration_never_overwrites_a_newer_summary_credential_and_the_consumer_reads_both_accounts(
    ) {
        let pool = summary_fixture().await;
        sqlx::query("INSERT INTO settings (id, provider, model, whisperModel, openaiApiKey, customOpenAIConfig) VALUES ('1', 'custom-openai', 'fixture', 'fixture', 'legacy-standard', ?)")
            .bind(r#"{"endpoint":"https://provider.example/v1","apiKey":"legacy-custom","model":"fixture"}"#)
            .execute(&pool)
            .await
            .unwrap();
        let credentials = MemorySummaryCredentials::default();
        credentials
            .save(&summary_credential_account("openai"), "newer-standard")
            .unwrap();
        ...
        let standard = load_summary_credential(&credentials, &LLMProvider::OpenAI)
            .unwrap()
            .expect("newer standard credential");
        let custom = load_summary_credential(&credentials, &LLMProvider::CustomOpenAI)
            .unwrap()
            .expect("migrated custom credential");
        assert_eq!(standard.expose(), b"newer-standard");
        assert_eq!(custom.expose(), b"legacy-custom");
    }
