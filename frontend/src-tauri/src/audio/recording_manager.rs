...
fn requires_initial_persistence(microphone_selected: bool, system_selected: bool) -> bool {
    microphone_selected || !system_selected
}
...
impl RecordingManager {
    ...
    pub(crate) fn set_microphone_participants(
        ...
        self.recording_saver.set_microphone_participants(snapshot);
    }
    ...
    pub async fn start_recording(
        ...
        // Preserve each selected source before the pipeline mixes it for transcription.
        let (recording_sender, persistence_ready) = self.recording_saver.start_accumulation(
            auto_save,
        ...
        // Start the audio pipeline. It persists each origin, mixes for
        // transcription, then applies VAD.
        ...
        if requires_initial_persistence(microphone_device.is_some(), system_device.is_some()) {
            let persistence_result =
                tokio::time::timeout(std::time::Duration::from_secs(3), persistence_ready).await;
            if persistence_result.is_err() || matches!(persistence_result, Ok(Err(_))) {
                let _ = self.recording_saver.set_capture_status("error");
                self.state.stop_recording();
                let _ = self.stream_manager.stop_streams();
                let _ = self.pipeline_manager.stop().await;
                return Err(anyhow::anyhow!(
                    "No selected audio source produced persistent samples"
                ));
            }
        } else {
            info!("System-only capture started; persistence waits for the first output sample");
        }
        if let Err(error) = self.recording_saver.set_capture_status("recording") {
            let _ = self.recording_saver.set_capture_status("error");
            self.state.stop_recording();
            let _ = self.stream_manager.stop_streams();
            let _ = self.pipeline_manager.stop().await;
    ...
    pub async fn save_recording_only<R: tauri::Runtime>(
        ...
        match self
            ...
            .await
        {
            Ok(Some(_)) => info!("Recording saved successfully"),
            Ok(None) => {
    ...
    pub fn get_audio_levels(&self) -> (f32, f32) {
        self.state.get_audio_levels()
    }

    pub fn get_source_statuses(&self) -> (&'static str, &'static str) {
        self.state.get_source_statuses()
    }
    ...
    pub async fn attempt_device_reconnect(
        ...
        info!("Attempting to reconnect {:?} device", device_type);
        ...
        if let Some(device) = device {
            info!("Device found, recreating stream");
            ...
            warn!("Device is not yet available");
            Ok(false)
        }
    }
    ...
    pub async fn handle_device_disconnect(
        ...
        warn!("{:?} device disconnected", device_type);
    ...
    pub async fn handle_device_reconnect(
        ...
        info!("{:?} device reconnected", device_type);
...
#[cfg(test)]
mod tests {
    use super::requires_initial_persistence;

    #[test]
    fn silent_system_only_capture_can_start_before_output_samples_arrive() {
        assert!(!requires_initial_persistence(false, true));
        assert!(requires_initial_persistence(true, false));
        assert!(requires_initial_persistence(true, true));
    }
}
