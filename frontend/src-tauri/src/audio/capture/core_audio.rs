...
impl CoreAudioCapture {
    ...
    pub fn new() -> Result<Self> {
        ...
        info!("Core Audio default output device resolved");
        ...
        match tap_asbd {
            Ok(asbd) => {
                info!("Core Audio process tap created");
                info!(
        ...
        let agg_desc = cf::DictionaryOf::with_keys_values(
            ...
            &[
                ...
                cf::str!(c"gcrdings-audio-tap").as_type_ref(),
                &output_uid,
...
mod tests {
    ...
    async fn test_core_audio_capture() {
        ...
        let seconds = std::env::var("TEST_CAPTURE_SECONDS")
            .ok()
            .and_then(|value| value.parse::<usize>().ok())
            .unwrap_or(1);
        let target_samples = stream.sample_rate() as usize * seconds;

        // Collect samples from the target Mac audio tap.
        let sample_count =
            tokio::time::timeout(std::time::Duration::from_secs(seconds as u64 + 10), async {
                let mut count = 0;
            ...
            .await
            .expect("Audio capture did not deliver samples within the allowed tolerance");

        info!("Collected {} samples", sample_count);
        assert_eq!(sample_count, target_samples, "Audio capture ended early");
    }
}
