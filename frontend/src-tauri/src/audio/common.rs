...
/// Unload the transcription engine when batch processing no longer needs it.
/// Skips unloading if a live recording is currently in progress, since recording
/// uses the same global engine instances.
pub(crate) async fn unload_transcription_engine(use_parakeet: bool) {
    let _engine_lifecycle_guard = acquire_engine_lifecycle_lock().await;

    if crate::audio::recording_commands::is_recording().await {
        log::info!("Skipping transcription model unload: recording in progress");
        return;
    }
    ...
    let json = serde_json::json!({
        ...
        "segments": segments.iter().enumerate().map(|(i, s)| {
            serde_json::json!({
                ...
                "ambiguity_group_id": s.ambiguity_group_id,
                "alignment_decision": s.alignment_decision,
...
mod tests {
    ...
    fn transcript_json_preserves_speaker_labels() {
        ...
        assert_eq!(saved["segments"][0]["speaker"], "Speaker 1");
        assert_eq!(saved["segments"][0]["source_origin"], "imported");
        assert_eq!(
            saved["segments"][0]["speaker_cluster_id"],
            "imported:Speaker 1"
        );
    }
}
