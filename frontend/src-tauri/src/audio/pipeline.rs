...
impl AudioMixerRingBuffer {
    ...
    fn add_samples(&mut self, device_type: DeviceType, samples: Vec<f32>) {
        ...
        unsafe {
            SAMPLE_COUNTER += 1;
            if SAMPLE_COUNTER.is_multiple_of(200) {
                debug!(
...
impl AudioCapture {
    pub fn new(
        ...
        if needs_resampling {
            ...
            warn!(
                "{:?} audio source ({:?}) reports {} Hz; resampling to {} Hz",
            ...
            info!(
                "{:?} audio source ({:?}) uses {} Hz",
                device_type, device_kind, sample_rate
            );
        }
        ...
        let (noise_suppressor, high_pass_filter, normalizer) = if matches!(
            ...
            let ns = if super::ffmpeg_mixer::RNNOISE_APPLY_ENABLED {
                match NoiseSuppressionProcessor::new(TARGET_SAMPLE_RATE) {
                    Ok(processor) => {
                        info!("Microphone noise suppression enabled");
                        Some(processor)
                    }
                    Err(e) => {
                ...
                info!("Microphone noise suppression disabled");
            ...
            let hpf = {
                ...
                info!("Microphone high-pass filter initialized");
                Some(filter)
            };
            ...
            let norm = match LoudnessNormalizer::new(1, TARGET_SAMPLE_RATE) {
                Ok(normalizer) => {
                    info!("Microphone loudness normalizer initialized");
                    Some(normalizer)
                }
                Err(e) => {
                    warn!(
            ...
            info!("System audio captured without enhancement");
            (None, None, None)
        };
        ...
        let resampler = if needs_resampling {
            ...
            match SincFixedIn::<f32>::new(
                ...
                Ok(resampler) => {
                    info!(
                        "Persistent resampler initialized ({}Hz → {}Hz, chunk_size={})",
                        sample_rate, TARGET_SAMPLE_RATE, RESAMPLER_CHUNK_SIZE
                    );
    ...
    pub fn process_audio_data(&self, data: &[f32]) {
        ...
        // Each origin enters the pipeline once. The pipeline persists it before
        // mixing a separate stream for transcription.
    ...
    pub fn handle_stream_error(&self, error: cpal::StreamError) {
        error!("Audio source stream error: {}", error);
        ...
        {
            warn!("Audio source disconnect detected");
        ...
        self.state
            .report_source_error(self.device_type.clone(), audio_error);
    }
}
...
pub struct AudioPipeline {
    ...
    // Recording sender preserves each source before mixing.
    recording_sender: Option<mpsc::UnboundedSender<AudioChunk>>,
}

impl AudioPipeline {
    // The constructor binds the existing independent audio-channel collaborators.
    ...
    pub fn new(
        ...
        info!(
            "Audio pipeline initialized with {:?}/{:?} source classes",
            mic_device_kind, system_device_kind
        );
        ...
        loop {
            ...
            .await
            {
                Ok(Some(chunk)) => {
                    ...
                    if let Some(sender) = &self.recording_sender {
                        if sender.send(chunk.clone()).is_err() {
                            warn!("Audio origin persistence stopped before capture ended");
                        }
                    }
...
impl AudioPipelineManager {
    ...
    // The start boundary mirrors the established per-device capture contract.
    #[allow(clippy::too_many_arguments)]
    pub fn start(
        &mut self,
        state: Arc<RecordingState>,
        ...
        pipeline.recording_sender = recording_sender;
        ...
        info!("Audio pipeline manager started with origin persistence");
        Ok(())
    }
