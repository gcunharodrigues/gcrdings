...
fn convert_to_wav_with_ffmpeg(
    ...
    info!(
        "Converting .{} to temporary WAV via verified FFmpeg",
        input_path
            .extension()
            .and_then(|e| e.to_str())
            .unwrap_or("unknown")
    ...
    if whisper_compatible {
        command.args(["-ac", "1", "-ar", "16000"]);
    }
    command
        .args(["-acodec", "pcm_s16le", output_str])
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
    ...
    if !output.status.success() {
        error!("FFmpeg conversion failed (exit code: {})", output.status);
        return Err(anyhow!(
...
/// Decode through a bounded, private 16 kHz mono intermediate for transcription.
pub fn decode_audio_file_for_transcription(path: &Path) -> Result<DecodedAudio> {
    let temporary = convert_to_wav_with_ffmpeg(path, None, true)?;
    decode_audio_file_with_progress(&temporary, None)
}
...
pub fn decode_audio_file_with_progress(
    ...
    info!("Decoding local audio file");
    ...
    let (_temp_wav_guard, decode_path): (Option<tempfile::TempPath>, Cow<'_, Path>) =
        if needs_ffmpeg_conversion(path) {
            ...
            let temp_path = convert_to_wav_with_ffmpeg(path, progress_callback.as_ref(), false)?;
...
mod tests {
    ...
    #[test]
    fn compatible_mono_audio_reuses_its_allocation() {
        let audio = DecodedAudio {
        ...
        let allocation = audio.samples.as_ptr();
    ...
    #[test]
    fn transcription_decode_is_16khz_mono_and_preserves_source() {
        let directory = tempfile::tempdir().unwrap();
        let source = directory.path().join("source.mp4");
        let samples = vec![0.25_f32; 48_000 * 2];
        ...
        .unwrap();
        let source_hash = Sha256::digest(std::fs::read(&source).unwrap());

        let decoded = decode_audio_file_for_transcription(&source).unwrap();

        assert_eq!(decoded.sample_rate, 16_000);
        assert_eq!(decoded.channels, 1);
        assert!((16_000..16_100).contains(&decoded.samples.len()));
        assert_eq!(Sha256::digest(std::fs::read(&source).unwrap()), source_hash);
    }
