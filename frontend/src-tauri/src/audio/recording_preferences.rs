...
pub struct MicrophoneParticipantSnapshot {
    pub local_speaker_name: String,
    pub microphone_is_shared: bool,
}

impl MicrophoneParticipantSnapshot {
    pub fn from_preferences(preferences: &RecordingPreferences) -> Self {
        Self {
            local_speaker_name: preferences.local_speaker_name.clone(),
            microphone_is_shared: preferences.microphone_is_shared,
        }
    }

    pub fn individual_speaker_name(&self) -> Option<String> {
        if self.microphone_is_shared {
            None
        } else {
            let name = self.local_speaker_name.trim();
            Some(if name.is_empty() { "You" } else { name }.to_string())
        }
    }
}
...
impl RecordingPreferences {
    pub fn individual_speaker_name(&self) -> Option<String> {
        MicrophoneParticipantSnapshot::from_preferences(self).individual_speaker_name()
    }
}
...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\gcrdings-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("gcrdings-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("gcrdings-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/gcrdings-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("gcrdings-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/gcrdings-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("gcrdings-recordings")
    }
...
fn validate_save_folder(path: &PathBuf) -> Result<()> {
    if path != &get_default_recordings_folder() {
        return Err(anyhow::anyhow!(
            "Custom recording folders are not supported"
        ));
    }
    Ok(())
}
...
pub async fn load_recording_preferences<R: Runtime>(
        ...
        match serde_json::from_value::<RecordingPreferences>(value.clone()) {
            Ok(mut p) => {
                ...
                #[cfg(target_os = "macos")]
                {
                    ...
                    p.system_audio_backend = Some(backend.identifier().to_string());
                }
                p
            }
            Err(e) => {
    ...
    // V1 uses one app-controlled library root. A renderer cannot authorize
    // arbitrary filesystem roots by editing stored preferences.
    prefs.save_folder = get_default_recordings_folder();
    info!("Loaded recording preferences");
    Ok(prefs)
}
...
pub async fn save_recording_preferences<R: Runtime>(
    ...
    validate_save_folder(&preferences.save_folder)?;
    info!("Saving recording preferences");
...
#[cfg(test)]
mod captured_timeline_tests {
    use super::*;

    #[test]
    fn individual_microphone_uses_configured_participant_or_safe_default() {
        let configured = RecordingPreferences {
            local_speaker_name: "Guilherme".to_string(),
            ..Default::default()
        };
        assert_eq!(
            configured.individual_speaker_name().as_deref(),
            Some("Guilherme")
        );

        let blank = RecordingPreferences {
            local_speaker_name: "  ".to_string(),
            ..Default::default()
        };
        assert_eq!(blank.individual_speaker_name().as_deref(), Some("You"));

        let shared = RecordingPreferences {
            microphone_is_shared: true,
            ..Default::default()
        };
        assert_eq!(shared.individual_speaker_name(), None);
    }
}
...
pub async fn get_available_audio_backends() -> Result<Vec<String>, String> {
    #[cfg(target_os = "macos")]
    {
        ...
        Ok(backends
            .iter()
            .map(|backend| backend.identifier().to_string())
            .collect())
    }
...
pub async fn get_current_audio_backend() -> Result<String, String> {
    ...
    {
        let backend = crate::audio::capture::get_current_backend();
        Ok(backend.identifier().to_string())
    }
...
mod tests {
    ...
    #[test]
    fn rejects_renderer_selected_library_roots() {
        assert!(validate_save_folder(&PathBuf::from("/tmp")).is_err());
        assert!(validate_save_folder(&get_default_recordings_folder()).is_ok());
    }

    #[test]
    fn individual_microphone_uses_the_configured_local_name() {
        let preferences = RecordingPreferences {
            local_speaker_name: "Guilherme".to_string(),
            ..RecordingPreferences::default()
        };
        assert_eq!(
            preferences.individual_speaker_name().as_deref(),
            Some("Guilherme")
        );
    }

    #[test]
    fn shared_microphone_requires_speaker_discovery() {
        let preferences = RecordingPreferences {
            microphone_is_shared: true,
            ..RecordingPreferences::default()
        };
        assert_eq!(preferences.individual_speaker_name(), None);
    }
}
