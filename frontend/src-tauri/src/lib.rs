    ...
    builder
        ...
        .setup(|_app| {
            ...
            if let Some(state) = _app.try_state::<state::AppState>() {
                let provider_credentials_ready = tauri::async_runtime::block_on(
                    providers::repository::ProviderRepository::migrate_legacy_credentials_on_startup(
                        state.db_manager.pool(),
                        &providers::keychain::MacKeychain::default(),
                    ),
                );
                if !provider_credentials_ready {
                    log::warn!(
                        "Legacy provider credential migration deferred because the macOS Keychain is unavailable"
                    );
                } else if let Err(error) = tauri::async_runtime::block_on(
                    database::manager::DatabaseManager::complete_legacy_import(_app.handle()),
                ) {
                    log::warn!("Legacy database import completion was deferred: {error}");
                }
                if let Err(error) = tauri::async_runtime::block_on(
                    providers::repository::ProviderRepository::recover_unresolved_transfers(
                        state.db_manager.pool(),
                    ),
                ) {
                    tracing::warn!(?error, "Provider transfer recovery was deferred");
                }
                let recovered = tauri::async_runtime::block_on(
                    database::repositories::processing_job::ProcessingJobsRepository::recover_interrupted(
                        state.db_manager.pool(),
                    ),
                )
                .expect("Failed to recover interrupted processing jobs");
                if recovered > 0 {
                    log::info!("Recovered {recovered} interrupted processing job(s)");
                }
                let recovered = tauri::async_runtime::block_on(
                    verifiable_record::repository::VerifiableRecordRepository::recover_interrupted(
                        state.db_manager.pool(),
                    ),
                )
                .expect("Failed to recover interrupted local findings generations");
                if recovered > 0 {
                    log::info!("Recovered {recovered} interrupted local findings generation(s)");
                }
            }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            api::api_get_session_record,
            api::api_read_session_audio,
            api::api_save_meeting_title,
            api::api_save_session_record,
            agent_handoff::api_export_agent_handoff,
            providers::commands::api_get_provider_status,
            providers::commands::api_save_provider_credential,
            providers::commands::api_test_provider_credential,
            providers::commands::api_remove_provider_credential,
            providers::commands::api_set_provider_task_enabled,
            ...
            audio::diarization_models::diarization_model_status,
            audio::diarization_models::diarization_install_models,
