...
pub struct ModelStatus {
    ...
    pub summary: String,  // "available" for the system Foundation Models capability
...
pub async fn complete_onboarding<R: Runtime>(
    ...
    if let Err(e) =
        SettingsRepository::save_model_config(pool, "apple-foundation", "system", "large-v3", None)
            .await
    {
        error!("Failed to save local findings config: {}", e);
        return Err(format!("Failed to save local findings config: {}", e));
    }
    ...
    status.model_status.summary = "available".to_string();
    status.model_status.selected_summary_model = Some("apple-foundation".to_string());
