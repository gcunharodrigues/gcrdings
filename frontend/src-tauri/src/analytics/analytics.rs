...
/// Env var carrying a gcrdings-owned PostHog project key. gcrdings ships no
/// telemetry credential of its own, so an absent var must build a config that
/// sends nothing (see `AnalyticsClient::new`, which no-ops on an empty key).
pub const ANALYTICS_API_KEY_ENV: &str = "GCRDINGS_ANALYTICS_API_KEY";

fn analytics_config(api_key: String) -> AnalyticsConfig {
    AnalyticsConfig {
...
pub fn analytics_config_from_env() -> AnalyticsConfig {
    analytics_config(std::env::var(ANALYTICS_API_KEY_ENV).unwrap_or_default())
}
...
impl AnalyticsClient {
    ...
    pub async fn identify(
        ...
        *self.user_id.lock().await = Some(user_id.clone());
        ...
        if let Err(e) = client.capture_immediate(event).await {
    ...
    // Analytics event fields intentionally mirror the stable external event schema.
...
mod tests {
    ...
    #[tokio::test]
    async fn absent_analytics_key_yields_disabled_client() {
        let config = analytics_config(String::new());
