use crate::analytics::{analytics_config_from_env, AnalyticsClient};
...
pub async fn init_analytics() -> Result<(), String> {
    let config = analytics_config_from_env();
...
#[command]
// Tauri IPC fields intentionally mirror the stable analytics command contract.
