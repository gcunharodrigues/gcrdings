...
A modern desktop application for recording, transcribing, and analyzing sessions with AI assistance. Built with Next.js and Tauri for a native desktop experience.
...
2. Clone the gcrdings repository and navigate to the frontend directory:
   ```bash
   cd gcrdings/frontend
   ```
...
Current gcrdings does not require a separate FastAPI service, Docker backend, or manually started whisper-server process. Local transcription is handled by the Rust/Tauri desktop app.
